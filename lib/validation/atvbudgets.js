 ...
 * @return array
 */
const validateATVBudgets = async(data) => {
  ...
  let request = supplierData.map((row) => {
    return new Promise((resolve, reject) => {
      ...
      /**
         ...
         * @param row.basket_cost_with_returns number required
         */
      ...
      if (!row.basket_cost_with_returns && row.basket_cost_with_returns !== 0) {
        result.errorID = '';
        result.errorField = 'basket_cost';
        result.errorReason = 'Missing Required Property';
        reject(result);
      } else {
        if (isNaN(row.basket_cost_with_returns)) {
          result.errorID = '';
          result.errorField = 'basket cost';
          result.errorReason = 'Not a Number!';
          reject(result);
        } else {
          row.metrics = { 'basket_cost_with_returns': row.basket_cost_with_returns };
        }
      }
